  ...
  page.controller('gridController', ['$scope','$window', function ($scope,$window) {
    ...
    $scope.showBox = function (box) {
      if ($scope.bigBox == box) {
        $scope.openBox = true;
        if ($scope.mobile) {
          $scope.boxes[((box.X) * $scope.gridWidth) + (box.Y)].T = box.X * 20;
          $scope.boxes[((box.X) * $scope.gridWidth) + (box.Y)].L = box.Y * 20;
        ...
        if ($scope.mobile) {
          ...
          $scope.boxes[((box.X) * $scope.gridWidth) + (box.Y)].T = 0;
          $scope.boxes[((box.X) * $scope.gridWidth) + (box.Y)].L = 0;
        }else {
    ...
    $scope.zoomOut = function (box) {
      if ($scope.bigBox != box && !$scope.mobile) {
        if (box.X > 0 && box.Y > 0 && box.X < ($scope.gridWidth - 1) && box.Y < ($scope.gridHeight - 1)) {
